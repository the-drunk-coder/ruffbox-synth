...
use crate::building_blocks::{
    ...
    ValueOrModulator, FilterType, OscillatorType
};

/// a triangle synth with envelope etc.
pub struct SingleOscillatorSynth<const BUFSIZE: usize, const NCHAN: usize> {
    oscillator: Box<dyn MonoSource<BUFSIZE>>,
    lp_filter: Box<dyn MonoEffect<BUFSIZE>>,
    hp_filter: Box<dyn MonoEffect<BUFSIZE>>,
...
impl<const BUFSIZE: usize, const NCHAN: usize> SingleOscillatorSynth<BUFSIZE, NCHAN> {
    pub fn new(osc_type: OscillatorType, lp_type: FilterType, hp_type: FilterType, sr: f32) -> Self {
    ...
    for SingleOscillatorSynth<BUFSIZE, NCHAN>
{
    ...
    fn get_next_block(
        ...
        out = self.lp_filter.process_block(out, start_sample, sample_buffers);
	out = self.hp_filter.process_block(out, start_sample, sample_buffers);
        out = self
            .envelope
